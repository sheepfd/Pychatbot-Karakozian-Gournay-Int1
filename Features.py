...
for filename in os.listdir("Cleaned"):
    ...
    for key,value in speech.items():
        if key == "nation":
            print(filename) # Output: Nomination_Chirac1, Nomination_Chirac2, Nomination_Hollande, Nomination_Macron, Nomination_Mitterrand2
            if max < value:
                max = value
...
"""for filename in os.listdir("Cleaned"):
    ...
    for key in speech.keys():
        if key == "climat" or key == "écologie":
            print(filename) # Output: Nomination_Macron he is the only one thus he is at same the time the first one """
#6
all_speeches = ""
for filename in os.listdir("Cleaned"):
    with open(f"Cleaned/{filename}", "r", encoding="utf-8") as f:
