...
def lower_letter(file_name):
    ...
    with open(f"Cleaned/{file_name.split('.')[0].split('/')[-1]} Cleaned version ","a") as f: #Create a New file with the New content
        f.write(contentV1)
    contentV2 = "" #let's initialize a new variable in oder to change again and only have lower letter and nothing else
    for  i in contentV1:
        if ord(i) >95 and ord(i)<122 or ord(i)==32:
            contentV2 = contentV2 + i
    with open(f"Cleaned/{file_name.split('.')[0].split('/')[-1]} Cleaned version ",
              "w") as f:  # change the file with the New content
        f.write(contentV2)
